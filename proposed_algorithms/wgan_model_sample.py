...
from utils.data_loader import load_data, normalize_data
...
class WGAN(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # #
        #
        # self.proposed_algorithms = nn.Sequential(self.map1, nn.Tanh(),
        #                            self.map2,nn.Tanh(),
        #                            self.map3
        #                            )
    ...
    #     y_preds = self.proposed_algorithms(x)
    #
    #     return y_preds
    #
    #     # x = F.elu(self.map1(x))
    ...
    #             y_preds = self.proposed_algorithms(batch_x)
    #             # print(y_preds)
    #             # y_preds=self.proposed_algorithms.forward(batch_x)
    ...
    #
    #             self.proposed_algorithms.zero_grad()
    #             loss.backward()
    #
    #             # # Update the weights using gradient descent. Each parameter is a Tensor, so
    #             # # we can access and gradients like we did before.
    #             # with torch.no_grad():
    #             #     for param in self.proposed_algorithms.parameters():
    #             #         param -= self.learning_rate * param.grad
    #
    #             self.optimizer.step()
    #
    #             # Weight Clipping
    #             for p in self.proposed_algorithms.parameters():
    #                 p.input_data.clamp_(-0.01, 0.01)
...
if __name__ == '__main__':
    ...
    input_file = '../input_data/ids2017_sampled.csv'
    output_file = '../input_data/ids_selected_features_data.csv'
    #
    ...
    for i in range(2):
        ...
        wgan = WGAN((training_set[0], training_set[1]), nn_size_lst, mini_batch_size, epochs, show_flg, data_flg,
                    critic=3, distance_threshold=0.05)
        ...
        ### test
        normal_data = []
        # achieve mini_batch_size normal input_data to compute the average of D
        for j in range(len(training_set[1])):
            if int(training_set[1][j].tolist()) == 1:  # normal input_data
    ...
    ## merge files
    all_file_lst = [os.path.join(root_dir, str(percent) + '_all_in_one_file.csv.arff'),
                    os.path.join(root_dir, str(percent) + '_training_set.csv'),
                    os.path.join(root_dir, str(percent) + '_gen_benign_data.csv'),
                    os.path.join(root_dir, str(percent) + '_gen_attack_data.csv')]
