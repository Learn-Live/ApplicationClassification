...
class WGAN(nn.Module):
    def __init__(self, num_classes=2, num_features=60, batch_size=30):
        ...
        :param num_features: 60= 40 (hearder) +20 (payload)
        """
    ...
    def run_train(self, train_loader, mode=True):
        ...
        for epoch in range(num_epochs):
            ...
            for i, (b_x, b_y) in enumerate(train_loader):
                ...
                if ((i + 1) % self.n_critic) == 0:
                    ...
                    # self.train_hist['G_loss'].append(-G_loss.input_data[0])
                    # self.train_hist['G_loss'].append(D_real_loss.input_data - (-G_loss.input_data))

                    G_loss.backward()
                    self.g_optimizer.step()
                    # self.train_hist['D_loss'].append(D_loss.input_data[0])
                    # self.train_hist['D_loss'].append(wgan_distance.input_data[0])
                    # self.train_hist['D_loss'].append(wgan_distance.input_data)
                    # self.train_hist['D_decision'].append([D_real_loss.input_data, D_fake_loss.input_data, -G_loss.input_data])
                ...
                #            wgan_distance.input_data[0], D_real_loss.input_data[0], D_fake_loss.input_data[0], -G_loss.input_data[0]))
                # len_tmp = len(self.train_hist['D_loss'])
                # if len_tmp > 1 and len_tmp % 10 == 0:
                #     accumulated_value = 0.0
                #     j = len_tmp - 10
                #     for i in range(10):
                #         accumulated_value += abs(self.train_hist['D_loss'][j + i].input_data.tolist())
        ...
        # torch.save(proposed_algorithms.state_dict(), 'proposed_algorithms.ckpt')

    def run_test(self, test_loader):
        # Test the proposed_algorithms

        self.eval()  # eval mode (batchnorm uses moving mean/variance instead of mini-batch mean/variance)
        r"""proposed_algorithms.eval(): Sets the module in evaluation mode.
        ...
        with torch.no_grad():
            ...
            # print('Evaluation Accuracy of the proposed_algorithms on the {} samples: {} %'.format(total, 100 * correct / total))
    ...
    #             print('b_y_preds_real', b_y_preds.input_data.tolist())
    #             _, predicted = torch.max(b_y_preds.input_data, 1)
    #             total += b_y.size(0)
    #             correct += (predicted == b_y).sum().item()
    #
    ...
    #             z_ = z_.view([len(b_y), -1])
    #             b_y_preds = self.D(z_)
    #             print('b_y_preds_fake', b_y_preds.input_data.tolist())
    #
    #             # if step == 0:
    #             #     cm = confusion_matrix(b_y, predicted, labels=[0, 1, 2, 3])
    #             #     sk_accuracy = accuracy_score(b_y, predicted) * len(b_y)
    ...
    #
    #     acc = correct / total
    #     # return acc, loss.input_data.tolist()
...
def run_main_cross_validation(i):
    ...
    #     # train_loader = torch.utils.input_data.DataLoader(dataset, batch_size, shuffle=True, num_workers=4)  # use all dataset
    #     train_loader = torch.utils.input_data.DataLoader(dataset, batch_size, shuffle=False, num_workers=4,
    #                                                sampler=train_sampler)
    #     X, y = get_loader_iterators_contents(train_loader)
    #     cntr = Counter(y)
    #     print('train_loader: ', len(train_idxs_k), ' y:', sorted(cntr.items()))
    #     global test_loader
    #     test_loader = torch.utils.input_data.DataLoader(dataset, batch_size, shuffle=False, num_workers=4,
    #                                               sampler=test_sampler)
    #     X, y = get_loader_iterators_contents(test_loader)
    #     cntr = Counter(y)
    #     print('test_loader: ', len(test_idxs_k), ' y:', sorted(cntr.items()))
    #
    #     proposed_algorithms = ConvNet(num_classes, num_features=i * 60 + i - 1).to(device)
    #     proposed_algorithms.run_train(train_loader)
    #     show_results(proposed_algorithms.results, i)
    #
    #     # proposed_algorithms.run_test(test_loader)
    #     acc_sum_tmp = np.sum(proposed_algorithms.results['test_acc'])
    #     if acc_sum < acc_sum_tmp:
    #         print('***acc_sum:', acc_sum, ' < acc_sum_tmp:', acc_sum_tmp)
    #         acc_sum = acc_sum_tmp
    #         # Save the proposed_algorithms checkpoint
    #         torch.save(proposed_algorithms.state_dict(), 'model_%d.ckpt' % i)
...
if __name__ == '__main__':
    ...
    input_file = '../input_data/attack_normal_data/attack_data.csv'
    attack_model, attack_test_loader = run_main(input_file, num_features=41)

    input_file = '../input_data/attack_normal_data/test_mixed.csv'
