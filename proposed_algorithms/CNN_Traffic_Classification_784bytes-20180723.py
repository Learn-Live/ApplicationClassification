...
class ConvNet(nn.Module):
    ...
    def achieve_sentence(self, sentences, first_n_pkts=2):
        ...
        for sentence_i in sentences:
            ...
            #     tmp_lst.append(
            #         sentence_i[(cnt - 1) * num_features + (cnt - 1): cnt * num_features + (cnt - 1)].input_data.tolist()[
            #         :num_features])
            #     # t = (cnt - 1) * 60 + (cnt - 1)
            #     if cnt == first_n_pkts:
            #         break
            #     cnt += 1
            # tmp_lst=sentence_i.input_data.tolist()[:first_n_pkts*num_features]
    ...
    def upsampling_data(self, b_x, b_y):
        ...
        #         if key == b_y_i.input_data.tolist():
        #             a[key].append(b_x_i)
