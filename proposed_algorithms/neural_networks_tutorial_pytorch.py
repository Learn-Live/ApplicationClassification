...
from utilities.preprocess import achieve_train_test_data
...
#
# The learnable parameters of a proposed_algorithms are returned by ``net.parameters()``
...
def load_sequence_data(first_n_pkts_input_file, separator=','):
    ...
    Y = label
    # new_data = normalize_data(np.asarray(input_data, dtype=float), range_value=[0, 1], eps=1e-5)
    X = data
