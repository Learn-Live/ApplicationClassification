...
for i in range(packetlength):
    v_1 = ks_2samp(X_train1[:, i], X_val1[:, i])[1]
    v_2 = ks_2samp(X_train1[:, i], X_test1[:, i])[1]
    v_3 = ks_2samp(X_val1[:, i], X_test1[:, i])[1]
...
for i in range(packetlength):
    v_1 = ks_2samp(X_train[:, i], X_val[:, i])[1]
    v_2 = ks_2samp(X_train[:, i], X_test[:, i])[1]
    v_3 = ks_2samp(X_val[:, i], X_test[:, i])[1]

    print(i, v_1, v_2, v_3)

stat_data(X_train[:, :packetlength], name='X_train')
stat_data(X_val[:, :packetlength], name='X_val')
stat_data(X_test[:, :packetlength], name='X_test')
...
class AccuracyHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        print('lr: ', K.eval(lr))
        print('get_config(): ', self.model.optimizer.get_config())
...
model.add(Conv1D(16, kernel_initializer='he_normal', kernel_size=3, activation="relu"))  # 32
...
model.add(Dense(8, kernel_initializer='he_normal', activation='softmax', activity_regularizer=l1(l=0.3)
                ))  # activity_regularizer=l1_l2(), ,activity_regularizer=l2()
